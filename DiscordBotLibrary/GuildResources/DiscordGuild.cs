...
namespace DiscordBotLibrary.GuildResources
{
    ...
    public sealed class DiscordGuild : Guild
    {
        ...
        {
            Channel? channel = Channels.FirstOrDefault(x => x.Id == channelId);
            if (channel is null)
            {
                string endpoint = RestApiEndpoints.GetChannelEndpoint(channelId, ChannelSubresource.None);
        ...
        public GuildScheduledEvent? GetScheduledEvent(ulong scheduledEventId)
            => GuildScheduledEvents.FirstOrDefault(x => x.Id == scheduledEventId);
        ...
        /// <summary>
        /// Only works on channels that are part of a guild.
        /// </summary>
        /// <returns></returns>
        ...
        public async Task<GuildInvite[]?> GetInvitesAsync()
        {
            CallerInfos callerInfos = CallerInfos.Create();
            if (!CheckPermissions(DiscordPermissions.ManageGuild))
            {
                return null;
            }

            string endpoint = RestApiEndpoints.GetGuildEndpoint(Id, GuildSubresource.Invites);
        ...
        private bool CheckPermissions(DiscordPermissions permissions)
        {
            ...
            DiscordGuild? guild = client.GetGuild(Id);
        ...
        {
            DiscordClient.Logger.Log(LogLevel.Info, $"Left guild: {Name}({Id})");

            string endpoint = $"/users/@me/guilds/{Id}";
            return await DiscordClient
                .GetDiscordClient()
                .RestApiLimiter
                .DeleteAsync(endpoint, CallerInfos.Create());
        }
    }
}
